...
const Card = ({ image: src, title, price, subTitle, id }) => {
  ...
  return (
    <>
      <div
        onClick={() => {
          if (!isInCart) return router.push(`/menu/${id}`);
        }}
        className={`${
          !isInCart && " hover:cursor-pointer hover:border-primary"
        } card border-[2px] border-gray-300 border-opacity-60 bg-base-100 p-4 shadow-xl `}
      >
            ...
            {!isInCart ? (
              ...
              <div className=" flex gap-2" onClick={stopPropagation}>
                ...
                <input
                  ...
                  disabled
                  className="  h-10 max-w-[3rem] rounded-full border border-gray-400 px-2 text-center outline-none hover:border-black focus:border-none focus:ring-1 focus:ring-primary disabled:cursor-not-allowed disabled:bg-gray-300 disabled:hover:border-none"
                />
                <RoundButton
